...
setup_selinux() {
    ...
    elif [ "${ID_LIKE:-}" = coreos ] || [ "${VARIANT_ID:-}" = coreos ] || ([ "${ID:-}" = fedora ] && [ "${VARIANT_ID:-}" = iot ]); then
        rpm_target=coreos
...
"
    ...
    policy_error=fatal
    if [ "$INSTALL_K3S_SELINUX_WARN" = true ] || [ "${ID_LIKE:-}" = coreos ] || [ "${VARIANT_ID:-}" = coreos ] || ([ "${ID:-}" = fedora ] && [ "${VARIANT_ID:-}" = iot ]); then
        policy_error=warn
    fi
    ...
    if ! $SUDO chcon -u system_u -r object_r -t container_runtime_exec_t ${BIN_DIR}/k3s >/dev/null 2>&1; then
        ...
        if [ -x /usr/sbin/transactional-update ] || [ "${ID_LIKE:-}" = coreos ] || [ "${VARIANT_ID:-}" = coreos ] || ([ "${ID:-}" = fedora ] && [ "${VARIANT_ID:-}" = iot ]); then
