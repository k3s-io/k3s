...
var _ = Describe("Verify Create", Ordered, func() {
	Context("Cluster :", func() {
		...
		It("Create file for auto import", func() {
			cmd := `echo "docker.io/library/mysql:latest" > test.txt`
			_, err := e2e.RunCmdOnNode(cmd, serverNodeNames[0])
			Expect(err).NotTo(HaveOccurred(), "failed: "+cmd)

			cmd = `cp ./text.txt /var/lib/rancher/k3s/agent/images`
			_, err = e2e.RunCmdOnNode(cmd, serverNodeNames[0])
			Expect(err).NotTo(HaveOccurred(), "failed: "+cmd)
		})
		It("Should be imported in containerd image store", func() {
			cmd := `k3s ctr images list | grep 'library/mysql'`
			Expect(e2e.RunCmdOnNode(cmd, serverNodeNames[0])).To(ContainSubstring("library/mysql"))
		})
	})
})
...
var _ = AfterSuite(func() {
	...
	if !failed || *ci {
		_, err := e2e.RunCmdOnNode("rm /var/lib/rancher/k3s/agent/images/example.txt", serverNodeNames[0])
		Expect(err).NotTo(HaveOccurred())
