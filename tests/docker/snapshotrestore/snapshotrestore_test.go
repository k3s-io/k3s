...
var _ = DescribeTableSubtree("Verify snapshots and cluster restores work", func(opts snapshotOptions) {
	// Need an inner Describe that is Ordered, so each Entry can run in parallel, but the testlet itself maintains order within its steps.
	Describe("cluster snapshot and restore workflow", Ordered, func() {
