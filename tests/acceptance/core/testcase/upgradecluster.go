...
func TestUpgradeCluster(g ginkgo.GinkgoTestingT) {
	...
	for _, ip := range ServerIPs {
		cmd := "sudo sed -i \"s/|/| INSTALL_K3S_VERSION=" + *util.UpgradeVersion +
			"/g\" /tmp/master_cmd"
		gomega.Eventually(func(g gomega.Gomega) {
			_, err := util.RunCmdOnNode(cmd, ip)
...
func upgradeServers(installType string, serverIPs []string) error {
	...
	for _, ip := range serverIPs {
		...
		go func(ip string) {
			...
			cmd := fmt.Sprintf(util.InstallK3sServer, installType)
			fmt.Printf("\nUpgrading server:  " + cmd)
			if _, err := util.RunCmdOnNode(cmd, ip); err != nil {
				errCh <- err
				mu.Lock()
...
func upgradeAgents(installType string, agentIPs []string) error {
	...
	for _, ip := range agentIPs {
		cmd := fmt.Sprintf(util.InstallK3sAgent, installType)
		wg.Add(1)
		go func(ip string) {
			defer wg.Done()
			defer ginkgo.GinkgoRecover()
