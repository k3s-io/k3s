...
func encryptionEnable(ctx context.Context, control *config.Control, enable bool) error {
	...
	if providers[len(providers)-1].Identity != nil && (providers[0].AESCBC != nil || providers[0].Secretbox != nil) && !enable {
		...
		if providers[1].AESCBC == nil && providers[2].AESCBC == nil && control.EncryptProvider == secretsencrypt.AESCBCProvider {
			return fmt.Errorf("cannot enable secrets encryption with AES-CBC key type, no keys found")
		}
		if providers[1].Secretbox == nil && providers[2].Secretbox == nil && control.EncryptProvider == secretsencrypt.SecretBoxProvider {
			return fmt.Errorf("cannot enable secrets encryption with SecretBox key type, no keys found")
		}
